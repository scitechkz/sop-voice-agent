...
// **FIX APPLIED HERE:** Dynamically determine the protocol (wss:// for HTTPS, ws:// otherwise)
const ws_protocol = (window.location.protocol === "https:") ? "wss://" : "ws://"; // <-- LINE 10: NEW PROTOCOL LOGIC
const ws_url = ws_protocol + window.location.host + "/ws/" + sessionId; // <-- LINE 11: MODIFIED URL
//const ws_url = "ws://127.0.0.1:8000/ws/" + sessionId;
let websocket = null;
let is_audio = false;
...
function connectWebsocket() {
    ...
    websocket.onmessage = function (event) {
        ...
        if (
            ...
            // ✅ Append typing indicator to bottom if not already there
            // NOTE: This logic is fine for the streaming chunks, as the typingIndicator is already at the bottom after user input.
...
// ❌ REMOVED: connectWebsocket(); ← Don't auto-connect
connectWebsocket(); // FIX: Connect on page load for text-only by default
...
function addSubmitHandler() {
    messageForm.onsubmit = function (e) {
        ...
        if (message) {
            ...
            messagesDiv.appendChild(p); // 1. Append the user message first
            messageInput.value = "";

            // ✅ FIX: Move typing indicator to the bottom AFTER the user message
            // This ensures it appears last in the DOM, right above the input field.
            if (typingIndicator.parentElement) {
